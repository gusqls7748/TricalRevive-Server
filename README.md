...
[![.NET Build](https://github.com/gusqls7748/TricalRevive-Server/actions/workflows/dotnet-build.yml/badge.svg)](https://github.com/gusqls7748/TricalRevive-Server/actions/workflows/dotnet-build.yml)
...
![GM 어드민 도구 스크린샷](admin-screenshot.png)
...
![뽑기 시스템 콘솔 로그](gacha-console-log.png)
...
![TCP 실시간 채팅 화면](tcp-chat-screenshot.png)
...
![GitHub Actions 워크플로우 실행 화면](github-actions-ci.png)
...
![Kibana APM Services 화면](kibana-apm-services.png)
...
- **Docker Desktop의 WSL2 가상 디스크(`docker_data.vhdx`)가 비대해져 C 드라이브 용량 부족 발생**
  → Docker를 오래 쓰다 보면 컨테이너/이미지를 지워도 WSL2의 vhdx 파일 자체는 자동으로 줄어들지 않아 129GB까지 비대해짐. `docker container prune`, `docker image prune`으로 안 쓰는 리소스를 먼저 정리한 뒤(볼륨은 데이터 유실 위험이 있어 제외), Docker Desktop과 WSL을 완전히 종료(`wsl --shutdown`)한 상태에서 `diskpart`의 `compact vdisk`로 물리적 파일 크기를 111GB 축소함(129GB → 17.6GB).
- **재부팅 후 체크리스트**: 컴퓨터를 재시작하거나 Docker Desktop을 껐다 켜면, `docker-compose`로 띄운 인프라 컨테이너(PostgreSQL/Redis/Elasticsearch/Kibana/APM Server)는 자동으로 재기동되지 않음. 아래 순서로 복구해야 함.
  1. `cd docker && docker compose up -d` — 인프라 컨테이너 재기동
  2. `kubectl delete pods --all` — 인프라가 없어 죽어있던 파드 강제 재시작
  3. `kubectl get pods` — 두 파드 모두 `Running`, `RESTARTS` 카운트가 더 안 올라가는지 확인
